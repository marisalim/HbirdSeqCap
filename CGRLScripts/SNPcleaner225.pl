...
my %opts = ('d'=>2, 'I'=>10, 'D'=>1000000, 'k'=>1, 'u'=>0, 'a'=>2, 'Q'=>10, 'S'=>1e-4, 'b'=>1e-100, 'f'=>0, 'e'=>1e-4, 'h'=>1e-4, 'F'=>0, 'H'=>0, 'L'=>0, 'A'=>undef,
...
Options:
	...
	-d INT   minimum site read depth s$opts{d}]
    ...
    -I INT   INDEL filtering window [$opts{I}]
...
getopts('d:D:k:u:a:Q:S:b:f:e:h:F:H:L:A:M:B:p:r:X:I:tgv2', \%opts);
...
while (<>) {
    ...
    if ($t[7] =~ /INDEL/) {
      ...
      if ($t[4] !~ m/,/) { 	  
        ...
        $length1 = length($t[3]) if length($t[3]) >= length($t[4]);
        $length1 = length($t[4]) if length($t[4]) > length($t[3]);
      } # if ($t[4] !~ m/,/) {
      else {
	my @in = split /,/, $t[4];
	push @in, $t[3];
	...
	$length1 = length $best;
      ...
      my $window = $opts{I};
      for (my $i = $pos - $window; $i <= $pos + $length1 + $window; $i++) {
	$indels{$chr}{$i}++;
      }
    }
...
while (<IN>) {
  ...
  my $pos = $a[1];
  if (! $indels{$chr}{$pos}) {
    print OUT $chr, "\t", $pos, "\t", $pos+1, "\n";
  } 
}
close IN;
close OUT;
system ("mv $bed2 $opts{B}");
